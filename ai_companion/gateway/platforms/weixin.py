...
Connects AI Companion to WeChat personal accounts via Tencent's iLink Bot API.
...
from ai_companion.gateway.platforms.helpers import MessageDeduplicator
...
def _safe_id(value: Optional[str], keep: int = 8) -> str:
    ...
    if not raw:
        return "?"
    if len(raw) <= keep:
        return f"{raw[: max(1, min(3, len(raw)))]}..."
    return f"{raw[:keep]}..."


def _mask_secret_for_log(value: Optional[str], keep: int = 4) -> str:
    raw = str(value or "").strip()
    if not raw:
        return ""
    if len(raw) <= keep * 2:
...
def _is_public_http_url(url: str) -> bool:
    """Return True when an outbound media URL resolves outside private ranges."""
    try:
        parsed = urlparse(url)
    except Exception:
        return False
    ...
    async with aiohttp.ClientSession(trust_env=True, connector=_make_ssl_connector()) as session:
        ...
        while time.time() < deadline:
            ...
            if status == "wait":
                ...
                print(f"\n微信连接成功，account_id={_mask_secret_for_log(account_id)}")
                return {
                    "account_id": account_id,
                    "token": token,
                    "base_url": base_url,
...
class WeixinAdapter(BasePlatformAdapter):
    """Native AI Companion adapter for Weixin personal accounts."""
    ...
    async def connect(self) -> bool:
        if not check_weixin_requirements():
            ...
            self._write_status(
                state="fatal",
                error_code="weixin_missing_dependency",
        ...
        if not self._token:
            ...
            self._write_status(
                state="fatal",
                error_code="weixin_missing_token",
                error_message=message,
            )
            logger.warning("[%s] %s", self.name, message)
            return False
        if not self._account_id:
            ...
            self._write_status(
                state="fatal",
                error_code="weixin_missing_account",
                error_message=message,
            )
            logger.warning("[%s] %s", self.name, message)
            return False
        ...
        self._write_status(state="connected", error_code=None, error_message=None)
    ...
    async def disconnect(self) -> None:
        ...
        self._write_status(state="disconnected", error_code=None, error_message=None)
    ...
    def _status_details(self) -> Dict[str, Any]:
        return {
            "account_id": _mask_secret_for_log(self._account_id),
            "account_id_hint": _safe_id(self._account_id),
    ...
    def _write_status(
        ...
        *,
        ...
        try:
            from ai_companion.gateway.status import write_runtime_status
            ...
            write_runtime_status(
                ...
                platform_details=self._status_details(),
            )
        except Exception:
            pass
            ...
            try:
                ...
                if ret not in (0, None) or errcode not in (0, None):
                    if ret == SESSION_EXPIRED_ERRCODE or errcode == SESSION_EXPIRED_ERRCODE:
                        self._write_status(
                            state="degraded",
                            error_code=SESSION_EXPIRED_ERRCODE,
                    ...
                    self._write_status(
                        state="degraded",
                        error_code=errcode or ret,
                        error_message=str(response.get("errmsg", "") or "getUpdates failed"),
                    )
        ...
        try:
            ...
            for media_path, is_voice in media_files:
                try:
                    ...
                    logger.warning("[%s] media delivery failed for %s: %s", self.name, Path(media_path).name, exc)
    ...
    async def _send_file(
        ...
        if caption:
            last_message_id = f"ai-companion-weixin-{uuid.uuid4().hex}"
            await _send_message(
                self._send_session,
                base_url=self._base_url,
                token=self._token,
