"""Slash command definitions and autocomplete for the AI Companion CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("update", "Update AI Companion Agent to the latest version", "Info",
               gateway_only=True),
...
def _resolve_config_gates() -> set[str]:
    ...
    if not gated:
        return set()
    try:
        from ai_companion._vendor.gw_cli.config import read_raw_config
        cfg = read_raw_config()
    except Exception:
        return set()
    result: set[str] = set()
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    """
    try:
        from ai_companion._vendor.gw_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /gw handler.
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from ai_companion._vendor.gw_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
