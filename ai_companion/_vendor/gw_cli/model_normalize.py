...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to AI Companion' canonical ids."""
    raw = (provider_name or "").strip().lower()
    if not raw:
        return raw
    try:
        from ai_companion._vendor.gw_cli.models import normalize_provider
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from ai_companion._vendor.gw_cli.models import normalize_copilot_model_id
