...
from ai_companion._vendor.gw_cli.config import get_hermes_home, get_config_path, read_raw_config
from ai_companion._vendor.gw_constants import OPENROUTER_BASE_URL
...
def get_anthropic_key() -> str:
    ...
    """
    from ai_companion._vendor.gw_cli.config import get_env_value
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from ai_companion._vendor.gw_cli.copilot_auth import resolve_copilot_token
...
# =============================================================================
# Auth Store — persistence layer for ~/.gw/auth.json
# =============================================================================
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in AI Companion.
...
#
# Tokens live in ~/.gw/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `gw auth list` / `hermes status`."""
    try:
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.gw/auth.json (not ~/.codex/)
#
# AI Companion maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from AI Companion auth store (~/.gw/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to AI Companion auth store (~/.gw/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating AI Companion auth state."""
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from AI Companion's own Codex token store."""
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other AI Companion processes
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip AI Companion models — they're not reliable for agentic tool-calling
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `gw status` output.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    """
    # Check credential pool first — this is where `gw auth` and
    # `gw model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def _prompt_model_selection(
    ...
    """
    from ai_companion._vendor.gw_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from ai_companion._vendor.gw_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'gw model' or 'hermes setup' instead."""
...
def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.gw/auth.json."""

    # Check for existing AI Companion-owned credentials
    try:
    ...
    # Run a fresh device code flow — AI Companion gets its own OAuth session
    print()
    ...
    # Save tokens to AI Companion auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider("openai-codex", creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    print()
    print("Login successful!")
    from ai_companion._vendor.gw_constants import display_hermes_home as _dhh
