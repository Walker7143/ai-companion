...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to gw root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the AI Companion home directory."""
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        for fname in filenames:
            ...
            # Skip the output zip itself if it happens to be inside gw root
            try:
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a AI Companion backup.
    ...
    # Look for telltale files that a gw home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a gw dir name
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # gw_cli.profiles might not be available (fresh install)
...
# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and gw backup --quick)
# ---------------------------------------------------------------------------
...
def run_quick_backup(args) -> None:
    """CLI entry point for gw backup --quick."""
