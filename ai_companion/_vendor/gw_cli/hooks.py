"""gw hooks — inspect and manage shell-script hooks.
...
def hooks_command(args) -> None:
    """Entry point for ``gw hooks`` — dispatches to the requested action."""
...
# stdin a script sees under `gw hooks test` and `ai-companion hooks doctor`
...
def _cmd_test(args) -> None:
    from ai_companion._vendor.gw_cli.config import load_config
    from ai_companion._vendor.gw_cli.plugins import VALID_HOOKS
    from agent import shell_hooks
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    # is already allowlisted.  Otherwise `gw hooks doctor` would execute
