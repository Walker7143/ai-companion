...
# when hermes_constants is cached from a pre-update version during `gw update`.
...
SERVICE_DESCRIPTION = "AI Companion Agent Gateway - Messaging Platform Integration"
...
# Legacy service names from older AI Companion installs that predate the
# gw-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (gw-gateway-*.service) and unrelated third-party
# "gw" units are never matched.
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy AI Companion gateway units.
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy AI Companion gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy AI Companion gateway unit files if any are installed.
...
def remove_legacy_hermes_units(
    ...
    """Stop, disable, and remove legacy AI Companion gateway unit files.
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    # Default ~/.gw → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.gw → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.gw) — keep as-is
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (gw.service from pre-rename installs)
    # before installing the new gw-gateway.service. If both remain, they
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `gw gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def launchd_stop():
    ...
    # `gw gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def _setup_whatsapp():
    ...
    from ai_companion._vendor.gw_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from ai_companion._vendor.gw_cli.dingtalk_auth import dingtalk_qr_auth
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        # Stop, disable, and remove legacy AI Companion gateway unit files from
        # pre-rename installs (e.g. gw.service). Profile units and
        # unrelated third-party services are never touched.
