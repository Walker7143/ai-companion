...
from ai_companion._vendor.gw_cli.auth import AuthError, resolve_provider
from ai_companion._vendor.gw_cli.colors import Colors, color
from ai_companion._vendor.gw_cli.config import get_env_path, get_env_value, get_hermes_home, load_config
from ai_companion._vendor.gw_cli.models import provider_label
from ai_companion._vendor.gw_cli.nous_subscription import get_nous_subscription_features
from ai_companion._vendor.gw_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all AI Companion Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ AI Companion Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from ai_companion._vendor.gw_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    if not terminal_env:
        ...
        # (gw status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    try:
        from ai_companion._vendor.gw_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
