...
from ai_companion._vendor.gw_cli.nous_subscription import get_nous_subscription_features
...
# display_hermes_home imported lazily at call sites (stale-module safety during gw update)
...
from ai_companion._vendor.gw_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from ai_companion._vendor.gw_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from ai_companion._vendor.gw_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from ai_companion._vendor.gw_cli.plugins import _ensure_plugins_discovered
...
# between `gw tools` and `ai-companion setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    # Delegate to the shared gw model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from ai_companion._vendor.gw_cli.main import select_provider_and_model
    try:
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from ai_companion._vendor.gw_cli.auth_commands import auth_add_command
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'gw setup tts')."""
    _setup_tts_provider(config)
...
def _setup_weixin():
    ...
    from ai_companion._vendor.gw_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from ai_companion._vendor.gw_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from ai_companion._vendor.gw_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from ai_companion._vendor.gw_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from ai_companion._vendor.gw_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from ai_companion._vendor.gw_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from ai_companion._vendor.gw_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from ai_companion._vendor.gw_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_qqbot():
    ...
    from ai_companion._vendor.gw_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from ai_companion._vendor.gw_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from ai_companion._vendor.gw_cli.config
...
# Config values may have different semantics between OpenClaw and AI Companion.
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    # Execute the migration — overwrite=False so existing AI Companion configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing AI Companion config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
...
def run_setup_wizard(args):
    ...
    print(
        color(
            "│             ⚕ AI Companion Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your AI Companion Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
...
def _resolve_hermes_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``gw chat`` in a fresh process."""
