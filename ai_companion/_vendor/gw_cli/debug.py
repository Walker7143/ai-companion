"""``gw debug`` — debug tools for AI Companion Agent.
...
def _pending_file() -> Path:
    """Path to ``~/.gw/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``gw debug delete``
        # manually.
        pass
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from ai_companion._vendor.gw_cli.logs import LOG_FILES
...
def _capture_dump() -> str:
    """Run ``gw dump`` and return its stdout as a string."""
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``gw debug`` call.
    ...
    # best-effort — any failure is swallowed so ``gw debug`` stays
