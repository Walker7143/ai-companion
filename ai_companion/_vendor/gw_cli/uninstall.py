...
def remove_path_from_shell_configs():
    """Remove AI Companion PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing gw-agent or ai-companion PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# AI Companion Agent" comment and following line
                if '# AI Companion Agent' in line or '# gw-agent' in line:
                    skip_next = True
                    continue
                ...
                # Remove any PATH line containing gw
...
def remove_wrapper_script():
    """Remove the gw wrapper script if it exists."""
    wrapper_paths = [
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains gw_cli reference)
...
def uninstall_gateway_service():
    ...
    try:
        from ai_companion._vendor.gw_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from ai_companion._vendor.gw_cli.gateway import get_launchd_plist_path
...
def _discover_named_profiles():
    ...
    default root."""
    try:
        from ai_companion._vendor.gw_cli.profiles import list_profiles
    except Exception:
        return []
    try:
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m gw_cli.main` so we don't depend on a `gw`
...
def run_uninstall(args):
    ...
    print(color("│            ⚕ AI Companion Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.gw/, just remove the hermes-agent subdir
    ...
    # 5. Optionally remove ~/.gw/ data directory (and named profiles)
    if full_uninstall:
    ...
    print()
    print("Thank you for using AI Companion Agent! ⚕")
    print()
