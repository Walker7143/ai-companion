...
from ai_companion._vendor.gw_cli.default_soul import DEFAULT_SOUL_MD
...
def is_managed() -> bool:
    """Check if AI Companion is running in package-manager-managed mode.
...
def get_container_exec_info() -> Optional[dict]:
    ...
    from ai_companion._vendor.gw_constants import is_container
    if is_container():
        return None
...
# Re-export from ai_companion._vendor.gw_constants — canonical definition lives there.
from ai_companion._vendor.gw_constants import get_hermes_home  # noqa: F811,E402
...
def ensure_hermes_home():
    """Ensure ~/.gw directory structure exists with secure permissions.
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the AI Companion-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, AI Companion auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), AI Companion sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when AI Companion
        ...
        #  "/home/user/.gw/cache/documents:/output"]
    ...
    "browser": {
        ...
        "camofox": {
            # When true, AI Companion sends a stable profile-scoped userId to Camofox
    ...
    # single read_file page exceeds these limits, AI Companion truncates the
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.gw/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # This section is only needed for gw-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *AI Companion Agent*" header.
    ...
    # stored approval from ~/.gw/shell-hooks-allowlist.json.
    ...
    "code_execution": {
        ...
        #     gw-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.gw/logs/.
    ...
    # Session storage — controls automatic cleanup of ~/.gw/state.db.
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older AI Companion versions) write ``models`` as
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    # metadata.gw.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.gw/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.gw/config.yaml."""
    ensure_hermes_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — gw auth) — OpenAI Codex
#   nous         (OAuth — gw auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.gw/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.gw/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.gw/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.gw/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.gw/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.gw/.env into os.environ. Returns count of vars updated.
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known AI Companion vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.gw/.env or environment."""
