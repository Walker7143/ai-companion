...
def _skin_color(key: str, fallback: str) -> str:
    ...
    try:
        from ai_companion._vendor.gw_cli.skin_engine import get_active_skin
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active AI Companion git checkout, or None if this isn't a git install."""
    hermes_home = get_hermes_home()
    repo_dir = hermes_home / "hermes-agent"
    if not (repo_dir / ".git").exists():
        repo_dir = Path(__file__).parent.parent.resolve()
...
def format_banner_version_label() -> str:
    ...
    base = f"AI Companion Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    agent_name = _skin_branding("agent_name", "AI Companion Agent")
