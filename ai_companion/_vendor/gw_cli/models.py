...
def _codex_curated_models() -> list[str]:
    ...
    """
    from ai_companion._vendor.gw_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from ai_companion._vendor.gw_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  gw model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `gw model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `gw model` TUI
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — gw-agent
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return gw-shaped pricing.
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    try:
        from ai_companion._vendor.gw_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from ai_companion._vendor.gw_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to AI Companion' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether AI Companion should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from ai_companion._vendor.gw_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "openai-codex":
        ...
        # a AI Companion release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from ai_companion._vendor.gw_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from ai_companion._vendor.gw_cli.copilot_auth import copilot_request_headers
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: AI Companion' default Claude IDs elsewhere use
