...
app = FastAPI(title="AI Companion Agent", version=__version__)
...
# streamed to a per-action log file under ``~/.gw/logs/<action>.log`` so
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``gw <subcommand>`` detached and record the Popen handle.
...
async def restart_gateway():
    """Kick off a ``gw gateway restart`` in the background."""
    try:
...
async def update_hermes():
    """Kick off ``gw update`` in the background."""
    try:
...
# the canonical ``gw auth add <provider>`` command so the dashboard
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from ai_companion._vendor.gw_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same AI Companion-managed PKCE file
    ...
    if provider_id in ("anthropic", "claude-code"):
        ...
        try:
            from ai_companion._vendor.gw_cli.auth import clear_provider_auth
...
#          → persists to ~/.gw/.anthropic_oauth.json AND credential pool
...
# Guarded so gw web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both AI Companion file AND credential pool.
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    from ai_companion._vendor.gw_cli import auth as hauth
    if provider_id == "nous":
        from ai_companion._vendor.gw_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    from ai_companion._vendor.gw_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from ai_companion._vendor.gw_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from ai_companion._vendor.gw_cli.logs import _read_tail, LOG_FILES
...
async def get_skills():
    ...
    from ai_companion._vendor.gw_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from ai_companion._vendor.gw_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from ai_companion._vendor.gw_cli.tools_config import (
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.gw/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.gw/dashboard-themes/*.yaml for user-created themes.
