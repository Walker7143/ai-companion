...
from ai_companion._vendor.gw_cli import auth as auth_mod
...
def auth_add_command(args) -> None:
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `gw auth
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source AI Companion reads from
    ...
    from ai_companion._vendor.gw_cli.auth import suppress_credential_source
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `gw auth` is called bare."""
