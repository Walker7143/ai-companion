...
# Directories/files to exclude when exporting the default (~/.gw) profile.
...
# AI Companion subcommands that cannot be used as profile names/aliases
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from ai_companion._vendor.gw_cli.gateway import get_service_name, get_launchd_plist_path
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.gw itself — its parent is ~/ and its
        # directory name is ".gw", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.gw itself — disallow
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for gw profile names."""
    return '''# AI Companion Agent profile completion
# Add to ~/.bashrc: eval "$(gw completion bash)"
...
def generate_zsh_completion() -> str:
    """Generate a zsh completion script for gw profile names."""
    return '''#compdef gw
# AI Companion Agent profile completion
# Add to ~/.zshrc: eval "$(gw completion zsh)"
