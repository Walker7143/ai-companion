...
from ai_companion._vendor.gw_cli.providers import (
...
from ai_companion._vendor.gw_cli.model_normalize import (
...
_HERMES_MODEL_WARNING = (
    ...
    "for use with AI Companion Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research AI Companion 3 / AI Companion 4 chat families.
# The previous substring check (`"gw" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``gw-brain:qwen3-14b-ctx16k`` that just
# happen to carry "gw" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/AI Companion-3-Llama-3.1-70B, gw-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   gw-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous AI Companion 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous AI Companion 3/4 chat model."""
...
def switch_model(
    ...
    from ai_companion._vendor.gw_cli.runtime_provider import resolve_runtime_provider
    ...
    if explicit_provider:
        ...
        if pdef is None:
            ...
            try:
                from ai_companion._vendor.gw_cli.config import validate_config_structure
        ...
        if not new_model:
            if pdef.base_url:
                from ai_companion._vendor.gw_cli.runtime_provider import _auto_detect_local_model
    ...
    # Mirrors the same logic in gw_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    # Build curated model lists keyed by gw provider ID
    ...
    # --- 1. Check AI Companion-mapped providers ---
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # show up in the picker without requiring a AI Companion release.
    ...
    # --- 2. Check AI Companion-only providers (nous, openai-codex, copilot, opencode-go) ---
    from ai_companion._vendor.gw_cli.providers import HERMES_OVERLAYS
    from ai_companion._vendor.gw_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → AI Companion provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use AI Companion IDs ("copilot").
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # Resolve AI Companion slug — e.g. "github-copilot" → "copilot"
        ...
        if not has_creds:
            try:
                from ai_companion._vendor.gw_cli.auth import _load_auth_store
        ...
        if hermes_slug in {"copilot", "copilot-acp"}:
            ...
            # Use curated list — look up by AI Companion slug, fall back to overlay key
    ...
    # with `gw model`).
    try:
        from ai_companion._vendor.gw_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is AI Companion's canonical write key (matches
            ...
            # AI Companion writes ``models:`` as a dict keyed by model id
            # (see gw_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # AI Companion's own writer uses; a hyphen variant is accepted
            ...
            # active model. AI Companion's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, gw_cli/config.py) already consume that dict.
