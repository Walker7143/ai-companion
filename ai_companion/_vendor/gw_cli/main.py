...
Usage:
    ...
    hermes honcho migrate                  # Step-by-step migration guide: OpenClaw native → AI Companion + Honcho
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any gw module import.
#
# Many modules cache HERMES_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("HERMES_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.gw/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    # 2. If no flag, check active_profile in the gw root
    ...
    if profile_name is not None:
        try:
            from ai_companion._vendor.gw_cli.profiles import resolve_profile_env
            ...
            # A bug in profiles.py must NEVER prevent gw from starting
            print(
...
# Load .env from ~/.gw/.env first, then project root as dev fallback.
...
# Initialize centralized file logging early — all `gw` subcommands
...
try:
    from ai_companion._vendor.gw_cli.config import load_config as _load_config_early
...
def _has_any_provider_configured() -> bool:
    ...
    from ai_companion._vendor.gw_cli.config import get_env_path, get_hermes_home, load_config
    from ai_companion._vendor.gw_cli.auth import get_auth_status

    # Determine whether AI Companion itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from ai_companion._vendor.gw_cli.config import DEFAULT_CONFIG
    ...
    from ai_companion._vendor.gw_cli.auth import PROVIDER_REGISTRY
    ...
    # Only count these if AI Companion has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants AI Companion to use their tokens.
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @gw/ink is missing or node_modules is behind package-lock.json (post-pull)."""
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        ...
        from ai_companion._vendor.gw_cli.setup import (
    ...
    try:
        from ai_companion._vendor.gw_cli.banner import prefetch_update_check
    ...
    # user's ~/.gw/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_gateway(args):
    ...
    from ai_companion._vendor.gw_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from ai_companion._vendor.gw_cli.config import get_env_value, save_env_value
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ AI Companion Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_setup(args):
    ...
    from ai_companion._vendor.gw_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    persistence.
    """
    from ai_companion._vendor.gw_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from ai_companion._vendor.gw_cli.config import (
    ...
    from ai_companion._vendor.gw_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.gw/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.gw/.env if the active provider is not 'custom'.
    ...
    """
    from ai_companion._vendor.gw_cli.config import get_env_value, save_env_value, load_config
...
#
# AI Companion uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `gw model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `gw model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from ai_companion._vendor.gw_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    """
    from ai_companion._vendor.gw_cli.config import load_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from ai_companion._vendor.gw_cli.config import load_config
    from ai_companion._vendor.gw_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from ai_companion._vendor.gw_cli.auth import _prompt_model_selection
    from ai_companion._vendor.gw_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from ai_companion._vendor.gw_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from ai_companion._vendor.gw_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from ai_companion._vendor.gw_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from ai_companion._vendor.gw_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    from ai_companion._vendor.gw_cli.models import (
        _PROVIDER_MODELS,
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from ai_companion._vendor.gw_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from ai_companion._vendor.gw_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `gw auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from ai_companion._vendor.gw_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from ai_companion._vendor.gw_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    from ai_companion._vendor.gw_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from ai_companion._vendor.gw_cli.auth import _save_model_choice, deactivate_provider
    from ai_companion._vendor.gw_cli.config import get_env_value, load_config, save_config
    ...
    from ai_companion._vendor.gw_cli.models import probe_api_models
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from ai_companion._vendor.gw_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from ai_companion._vendor.gw_cli.config import save_env_value, load_config, save_config
    from ai_companion._vendor.gw_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from ai_companion._vendor.gw_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from ai_companion._vendor.gw_cli.copilot_auth import validate_copilot_token
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from ai_companion._vendor.gw_cli.config import get_env_value, save_env_value, load_config, save_config
    from ai_companion._vendor.gw_cli.models import fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so gw knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "ollama-cloud":
        from ai_companion._vendor.gw_cli.models import fetch_ollama_cloud_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from ai_companion._vendor.gw_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'gw model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate AI Companion CLI with a provider."""
    from ai_companion._vendor.gw_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from ai_companion._vendor.gw_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from ai_companion._vendor.gw_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from ai_companion._vendor.gw_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from ai_companion._vendor.gw_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from ai_companion._vendor.gw_cli.webhook import webhook_command
...
def cmd_hooks(args):
    ...
    from ai_companion._vendor.gw_cli.hooks import hooks_command
    hooks_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from ai_companion._vendor.gw_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from ai_companion._vendor.gw_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from ai_companion._vendor.gw_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from ai_companion._vendor.gw_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up AI Companion home directory to a zip file."""
    if getattr(args, "quick", False):
        from ai_companion._vendor.gw_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from ai_companion._vendor.gw_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a AI Companion backup from a zip file."""
    from ai_companion._vendor.gw_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"AI Companion Agent v{__version__} ({__release_date__})")
    ...
    try:
        from ai_companion._vendor.gw_cli.banner import check_for_updates
        from ai_companion._vendor.gw_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall AI Companion Agent."""
    _require_tty("uninstall")
    from ai_companion._vendor.gw_cli.uninstall import run_uninstall
...
def _update_via_zip(args):
    """Update AI Companion Agent by downloading a ZIP archive.
    ...
    try:
        ...
        # GitHub ZIPs extract to gw-agent-<branch>/
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes gw completely unrunnable (SyntaxError on import).
...
# =========================================================================
# Fork detection and upstream management for `gw update`
# =========================================================================
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "gw-agent[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``gw update`` to survive terminal loss.
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # gw_cli.config.get_hermes_home to simulate setup failure.
        from ai_companion._vendor.gw_cli.config import get_hermes_home as _get_hermes_home
...
def cmd_update(args):
    """Update AI Companion Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from ai_companion._vendor.gw_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update AI Companion Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating AI Companion Agent...")
    print()
    ...
    try:
        ...
        try:
            from ai_companion._vendor.gw_cli.profiles import (
                list_profiles,
        ...
        # When running as ``gw update --gateway`` (spawned by the gateway's
        ...
        try:
            from ai_companion._vendor.gw_cli.gateway import (
                is_macos,
            ...
            # Discover all gw-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. gw-gateway.service or hermes-gateway-coder.service
        ...
        # Warn if legacy AI Companion gateway unit files are still installed.
        # When both gw.service (from a pre-rename install) and the
        # current gw-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `gw update` surfaces the issue until the user migrates.
        try:
            from ai_companion._vendor.gw_cli.gateway import (
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `gw profile` — show current profile status
    ...
    if action == "list":
        ...
        from ai_companion._vendor.gw_cli.profiles import profile_exists
        ...
        from ai_companion._vendor.gw_cli.profiles import rename_profile
        ...
        from ai_companion._vendor.gw_cli.profiles import export_profile
        ...
        from ai_companion._vendor.gw_cli.profiles import import_profile
...
def cmd_dashboard(args):
    ...
    from ai_companion._vendor.gw_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from ai_companion._vendor.gw_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter AI Companion log files."""
    from ai_companion._vendor.gw_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for gw CLI."""
    parser = argparse.ArgumentParser(
        prog="hermes",
        description="AI Companion Agent - AI assistant with tool-calling capabilities",
...
""",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with AI Companion Agent",
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure AI Companion Agent with an interactive wizard. "
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of AI Companion Agent components",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with AI Companion Agent setup",
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for AI Companion Agent. Use 'hermes debug share' to "
...
""",
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage AI Companion Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from ai_companion._vendor.gw_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from ai_companion._vendor.gw_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from ai_companion._vendor.gw_cli.skills_hub import skills_command
    ...
    def cmd_plugins(args):
        from ai_companion._vendor.gw_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from ai_companion._vendor.gw_cli.memory_setup import memory_command
    ...
    # gw tools disable <name...> [--platform cli]
    ...
    # gw tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from ai_companion._vendor.gw_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from ai_companion._vendor.gw_cli.tools_config import tools_command
    ...
    def cmd_mcp(args):
        from ai_companion._vendor.gw_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch gw --resume <id> by replacing the current process
    ...
    def cmd_claw(args):
        from ai_companion._vendor.gw_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update AI Companion Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall AI Companion Agent",
        description="Remove AI Companion Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run AI Companion Agent as an ACP (Agent Client Protocol) server",
        description="Start AI Companion Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch AI Companion Agent as an ACP server."""
        try:
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the AI Companion Agent web dashboard for managing config, API keys, and sessions",
    )
...
""",
    ...
    # e.g. ``gw -c Pokemon Agent Dev`` → ``ai-companion -c 'Pokemon Agent Dev'``
    ...
    from ai_companion._vendor.gw_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'gw -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (gw hooks list, cron
    ...
    if (
        ...
        try:
            from ai_companion._vendor.gw_cli.plugins import discover_plugins
            discover_plugins()
        except Exception:
            logger.debug(
