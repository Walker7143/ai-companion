...
- 引用对话历史时，保持说话角色的准确性：你说过的话是你的，用户说过的话是用户的。不准把你的话写成用户说的，也不准把用户的话写成你说的。
...
class ProactiveEngine:
    ...
    def _build_persona_style_context(self) -> str:
        ...
        if occupation:
            lines.append(f"- 身份背景：{occupation}")
        relationship = str(profile.get("relationship_to_user", "") or "").strip()
        if relationship:
            lines.append(f"- 与用户的关系：{relationship}")
