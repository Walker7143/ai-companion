...
| Unified Vector Index | `vector/` + Chroma | 语义事实、用户理解、关系脉络、日摘要、Bot 人生轨迹的统一语义召回索引，可随时重建 |
...
AI Companion 现在采用“结构化数据库为事实源，Chroma 为语义索引”的混合架构。SQLite/JSON 仍然保存可编辑、可删除、可校验的原始记忆；Chroma 只负责“当前这句话语义上会联想到哪些背景”。

统一向量索引会从这些权威来源生成：

- `semantic.db`：用户事实、偏好、边界。
- `user_understanding.json`：用户理解里的 `manual` / `auto` / `relationship_memory` 投影。
- `relationship.db`：关系叙事、当前姿态、互动建议。
- `daily.db`：近日摘要和未完成话题。
- Bot 人生轨迹：日常事件和重要人生事件。

这些内容会作为 `semantic_fact`、`user_understanding`、`relationship_narrative`、`daily_summary`、`life_event`、`major_life_event` 等 source type 写入 `data/bots/{bot_id}/memory/vector/` 下的 Chroma collection：`unified_memory`。

“重建向量索引”只会重新生成 Chroma 检索索引，不会删除或改写 SQLite/JSON 中的原始记忆。适合在手动编辑记忆文件、换 embedding 模型、怀疑索引不同步时使用。
...
也可以在管理后台“记忆”页点击“重建向量索引”。如果后台页面来自 Vite 开发端口 `5173`，需要 gateway 已更新并重启，确保管理 API 默认 CORS 已放行 `http://localhost:5173`。
...
|------|------|
| `/new` | 开始新会话 |
| `/memory` | 查看工作记忆、情景记忆、用户事实、关系状态、用户理解文件路径和向量索引数量 |
...
**Q: 向量嵌入不生效**
A: 确认 `models.yaml` 中 `memory.embedding: "local"`（sentence-transformers 已默认安装），然后运行 `ai-companion memory rebuild-vector --bot <bot_id>`。如果在管理后台点击重建时报 `TypeError: Failed to fetch`，通常是 gateway 还没重启或浏览器跨域被拦截；执行 `ai-companion gateway restart` 后刷新页面再试。
...
本项目使用 [BSL 1.1（Business Source License）](./LICENSE) 许可证。

| 状态 | 说明 |
|------|------|
| **Change Date** | 2031-05-17 |
| **Change License** | 木兰宽松许可证第2版（MulanPSL-2） |
...
- 将本项目或其核心功能作为 SaaS 服务提供给第三方
- 搭建与本项目功能类似的托管平台并对外收费
- 转售或再分发本项目功能作为付费服务

上述限制在 Change Date（2031-05-17）自动解除，届时项目将转为完全开源的 MulanPSL-2 许可证。
