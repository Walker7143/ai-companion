...
class BotSkillCapabilityStatusTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_runtime_skills_view_includes_builtin_only(self):
        bot = BotInstance({"id": "shen_nian", "name": "???", "skills": {}}, model=None, memory_config=None)
        try:
            ...
            self.assertIn("image_generation", output)
            self.assertIn("image_understanding", output)
    ...
    async def test_external_skill_auto_route_is_removed(self):
        bot = BotInstance({"id": "shen_nian", "name": "???", "skills": {}}, model=EchoModel(), memory_config=None)
        try:
            bot._initialized = True
            bot._schedulers_started = True
            response = await bot.handle_message("?????????????????")
