...
主动消息成功发出后，会通过统一出口写入 Working / Daily 记忆，并带上：
...
`proactive_kind` 会随主动动机变化：普通空闲问候是 `idle_reminder`，延迟回复是 `deferred_reply`，接上文续聊是 `topic_continuation`，情绪跟进是 `emotion_followup`，生活事件分享是 `life_event`。

下一轮用户回复时，工作记忆会保留这份 `metadata_json`。如果上一条 assistant 消息是 Bot 主动发起的，生成上下文会补一条轻量连续性提示，让 Bot 知道“上一条是我主动发给用户的”，从而能自然承接用户的“在”“嗯”“怎么了”等短回复。Daily 记忆还会把这些 assistant-originated 消息投影成 `self_memory`，进入意识工作区里的“Bot 自己最近主动做过的事”，让她不仅记得用户说了什么，也记得自己刚刚为什么找过用户。

如果你想让 Bot 更像真人，建议优先开启 `延迟回复履约` 和 `接上文续聊`，把 `普通陪伴问候` 留作兜底。

普通陪伴问候只作为兜底。兜底文案会避免只有一个裸 `在吗` 的开场，优先使用带具体关系动机或上下文的表达；如果你在历史日志里看到 `...在吗？`，通常是旧版 fallback 生成的主动消息。
...
当前架构由长期存储、意图召回、意识工作区和 prompt 预算四段组成。它会完整保存值得保存的记忆，但每轮只把少量、相关、符合分寸的内容放进主模型 prompt：
...
用户消息 + Bot 回复
  ...
  → ConsciousContextBuilder 生成当轮意识工作区
  → MemoryPromptBuilder 按预算生成 system_suffix 和 diagnostics
```
...
| Conscious / 意识工作区 | 当前焦点、情绪读取、关系姿态、被唤起的少量记忆、本轮避免事项 | 不直接落库，由 `conscious.py` 每轮生成 | 只存在于本轮 prompt |
...
这套结构刻意贴近人类记忆：真人不会每句话都翻完整档案，而是由当下线索唤起少数记忆，再让更多长期理解只影响语气、边界和反应方向。

| 人类记忆层次 | Bot 对应实现 |
|--------------|--------------|
| 感知/瞬时记忆 | 当前输入、平台上下文、图片理解结果、Bot 时间线 |
| 工作记忆 | 最近几轮对话、压缩摘要、当前 session |
| 情景记忆 | `episodic.db` 中的共同经历、冲突/和解、承诺 |
| 语义记忆 | `semantic.db` 与 `user_understanding.json` 中的事实、偏好、边界 |
| 关系记忆 | `relationship.db` 中的关系状态、关键时刻、未完成情绪话题 |
| 自传体记忆 | `self_memory`、人生轨迹、Bot 自己发起过的消息和生活事件 |
| 前瞻记忆 | 主动唤醒待办、延迟回复、接上文续聊、情绪跟进 |
| 意识工作区 | `ConsciousContext`：本轮真正浮到“脑子里”的少量线索 |
...
`user_understanding.json` 可以保留完整理解，但主聊天不会把整份档案原样塞进 prompt。系统会按意图把理解投影成四层：

| 投影层 | 内容 | 进入 prompt 的原则 |
|--------|------|--------------------|
| `core` | 稳定身份、长期偏好、沟通方式、硬边界、关系期待 | 普通聊天也可少量常驻 |
| `current` | 近期状态、目标、未完成话题、压力源、作息变化 | 按当前意图进入 |
| `deep` | 情绪模式、有效陪伴方式、亲近/距离模式、价值观、生活背景 | 情绪支持、关系修复、回忆、主动关心时进入 |
| `sensitive` | 敏感线索和使用边界 | 普通聊天只作为“不要主动提起”的规则，用户主动提及时才谨慎使用 |

`MemoryPromptBuilder` 会给每一块上下文分配字符预算。普通闲聊更克制，情绪支持、关系修复和回忆会给更深层记忆更多空间；任务型请求则减少情感记忆干扰。这样记忆越多时，存储层可以继续增长，但单轮 prompt 不会跟着线性膨胀。

每次 `MemoryEngine.load_context()` 会返回 `memory_prompt_diagnostics`，包括：

- `max_chars`：记忆 suffix 的总预算。
- `raw_chars` / `final_chars`：截断前后字符数。
...
召回后的第一层不是直接拼 prompt，而是先生成 `ConsciousContext`。它通常包含：

- `current_focus`：本轮用户到底在推什么。
- `emotional_read`：应该先接住情绪，还是直接推进任务。
- `relationship_posture`：这一轮关系上该亲近、克制、修复还是放慢。
- `active_memories`：最多几条真正被唤起的共同经历、近期连续性、Bot 自传体线索或事实。
- `active_memory_details`：每条 active memory 的来源、激活分数和表达方式，表达方式包括 `silent_influence`、`light_reference`、`explicit_recall`、`ask_before_entering`、`avoid`。
- `avoid`：本轮不该主动触碰的敏感内容和表达方式。
- `recall_style`：记忆应该如何自然使用。
...
**A**: 新版主动消息会把 `assistant_initiated`、`proactive` 和 `proactive_kind` 写入 `working.db` / `daily.db` 的 `metadata_json`，用户回复后会自动接上这条主动消息，并通过 `self_memory` 投影成 Bot 自己最近主动做过的事。如果仍复现旧行为，优先排查：

1. `~/.ai-companion/logs/gateway.log` 里这条消息是否来自 `ProactiveEngine` fallback。
2. `~/.ai-companion/data/bots/{bot_id}/memory/working.db` 和 `daily.db` 中对应 assistant 消息是否有 `metadata_json`。
3. 当前服务是否已经重启到新版代码。

历史上单独的 `...在吗？` 一般来自旧版主动唤醒 fallback，不是记忆没落库。
...
- [类人记忆与 Token 控制设计](./DESIGN_human_like_memory_token_architecture.md)
