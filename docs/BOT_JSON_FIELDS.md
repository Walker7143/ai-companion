...
| 字段 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `daily_interval_seconds` | number | `3600` | 日常事件检查间隔，以 Bot 时间秒数表示。实际间隔会除以 `time_ratio`。 |
| `major_interval_seconds` | number | `21600` | 人生大事检查间隔，以 Bot 时间秒数表示。实际间隔会除以 `time_ratio`。 |
| `time_ratio` | number | `1` | Bot 时间流速倍率。值越大，Bot 日期推进越快。 |
...
系统内置日常场景 key（用于 `disabled_scenarios`、`scenario_weights` 或调试；不是必须配置项）：
